...
class _Pooling3D(Layer):
    ...
    def output_shape(self):
        ...
        if self.dim_ordering == 'th':
            ...
            raise Exception('TensorFlow not implemented: ' + self.dim_ordering)
        else:
            raise Exception('Invalid dim_ordering: ' + self.dim_ordering)

        depth = conv_output_length(depth, self.pool_size[0],
                                   self.border_mode, self.strides[0])
...
class MaxPooling3D(_Pooling3D):
    ...
    def _pooling_function(self, inputs, pool_size, strides,
                          border_mode, dim_ordering):
        # 2d pooling over first two dimensions
        output = K.pool2d(inputs.dimshuffle(0, 1, 4, 3, 2),
                          (pool_size[1], pool_size[0]),
                          (strides[1], strides[0]),
                          border_mode, dim_ordering, pool_mode='max')

        # 2d pooling over last two dimensions, where the second dimension remains constant
        output = K.pool2d(output.dimshuffle(0, 1, 4, 3, 2),
                          (1, pool_size[2]),
                          (1, strides[2]),
                          border_mode, dim_ordering, pool_mode='max')
        return output
